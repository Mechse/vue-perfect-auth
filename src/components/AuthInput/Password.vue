...
const hasAtLeastOneSpecialChar = (value: string) => {
  const re = /^(?=.*[^A-Za-z0-9])[^&\\;<>]+$/;
  return re.test(value)
}
const hasAtLeasOneNumber = (value: string) => {
  const re = /.*[0-9].*/;
  return re.test(value)
}
...
<template>
  <div>
    <div class="flex items-center justify-between">
      ...
      <a v-if="forgetPasswordLink !== ''" :href="forgetPasswordLink" title="" class="text-color text-sm font-medium">
        Forgot
        Password? </a>
    </div>
    <div class="mt-2 relative dark:text-gray-50 dark:bg-gray-700 dark:border-gray-700 rounded-lg">
      <input id="auth-password" v-model="state.password" @input="emit('update:password', state.password)"
        :type="showPassword ? 'text' : 'password'" name="" :placeholder="passwordPlaceholder"
      ...
      <div v-if="!showPassword"
        class="absolute top-3 right-0  mr-3 text-xl cursor-pointer text-gray-700 dark:text-gray-50 dark:bg-gray-700 dark:border-gray-700"
      ...
      <div v-else class=" mr-3 text-xl cursor-pointer text-gray-700 dark:text-gray-200 absolute top-3 right-0"
